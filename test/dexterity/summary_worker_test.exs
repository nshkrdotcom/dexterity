defmodule Dexterity.SummaryWorkerTest do
  ...
  test "bounded queue drops oldest pending work when full" do
    ...
    {:ok, server} =
      start_supervised(
        {SummaryWorker, [name: nil, db_conn: nil, enabled: true, max_queue: 1, llm_fn: llm_fn]}
      )

    SummaryWorker.summarize(server, "lib/#{marker}_a.ex", "A", marker, "sig-a")
    SummaryWorker.summarize(server, "lib/#{marker}_b.ex", "B", marker, "sig-b")
    SummaryWorker.summarize(server, "lib/#{marker}_c.ex", "C", marker, "sig-c")

    status = SummaryWorker.status(server)
  ...
  test "retries transient failures up to configured limit" do
    ...
    SummaryWorker.summarize(server, "lib/#{marker}_retry.ex", "RetryMod", marker, "sig")
